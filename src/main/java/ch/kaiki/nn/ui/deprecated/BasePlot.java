...
public abstract class BasePlot {
    ...
    BasePlot(GraphicsContext context) {
     ...
     */
    public BasePlot setPadding(double top, double right, double bottom, double left, double dataPadding) {
     ...
     * @return this plot instance (for chaining).
     */
    public BasePlot setFontProperties(boolean bold, boolean italic, double fontSize) {
        if (fontSize < 0) {
     ...
     *
     * @param colors the NNPlotColorDeprecated palette which will assign the base colors for this graph.
     * @return this plot instance (for chaining).
     */
    public BasePlot setColorPalette(NNPlotColorDeprecated colors) {
     ...
     * @return this plot instance (for chaining).
     */
    public BasePlot setTitle(String text) {
        this.title = text;
    ...
    void drawAxes(boolean drawAxes, boolean drawTicks, boolean drawAxisLabels) {
        ...
        if (drawTicks || drawAxisLabels) {
            ...
            for (int i = 0; i < xTickcount; i++) {
                ...
                if (x < wOffsetLeft + plotWidth - (plotWidth * padding * 0.1) && x > wOffsetLeft + (plotWidth * padding * 0.1)) {
                    drawText(xRangeLabel + i * xStep, x, height - hOffsetBottom + 20, TextAlignment.CENTER, drawAxisLabels);
            ...
            for (int i = 0; i < yTickcount; i++) {
                ...
                if (y > hOffsetTop + (plotHeight * padding * 0.1) && y < plotHeight + hOffsetTop - (plotHeight * padding * 0.1)) {
                    drawText(yRangeLabel + i * yStep, wOffsetLeft - 12, y + 4, TextAlignment.RIGHT, drawAxisLabels);
